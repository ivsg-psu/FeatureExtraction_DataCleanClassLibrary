function [flags, fitting_parameters, fit_sensors, mean_fit, filtered_median_errors] = fcn_DataClean_fitROSTime2GPSTime(dataStructure, varargin)
% fcn_DataClean_fitROSTime2GPSTime
% Calculated the model that fits GPS time to ROS time.
%
% FORMAT:
%
%      [flags, fitting_parameters, fit_sensors, mean_fit, filtered_median_errors] = fcn_DataClean_fitROSTime2GPSTime(...
...
for ith_array = 1:length(cell_array_ROS_Time)
    ...
    GPSfromROS_Time{ith_array} = fcn_DataClean_predictGPSTimeFromROSTime(mean_fit, filtered_median_errors, ROS_Time);
