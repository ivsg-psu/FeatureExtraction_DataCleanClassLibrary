function [nearbyIndicies, Nnearby] = fcn_DataClean_findNearPoints(ENU_data, searchRadiusAndAngles, varargin)
%fcn_DataClean_findNearPoints  for each point, lists nearby indicies
%
% FORMAT:
%
%       nearbyIndicies = fcn_DataClean_findNearPoints(fcn_DataClean_findNearPoints, searchRadiusAndAngles, (fig_num))
%
% INPUTS:
%
%      fcn_DataClean_findNearPoints: the [East North Up] data as an [Nx3]
...
%      to the nth point in the fcn_DataClean_findNearPoints input. The cell
...
if 1==flag_check_angles
    ...
    % [modeIndex, ~, offsetCentisecondsToMode] = fcn_DataClean_assessTime([], ENU_data, (-1));
    % 
    % % Calculate the velocities, angles, and heading
    % [~, angleENUradians, ~]  = fcn_DataClean_calcVelocity([], ENU_data, modeIndex, offsetCentisecondsToMode, -1); 
end
