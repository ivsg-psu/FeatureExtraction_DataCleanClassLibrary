%% script_test_fcn_DataClean_findNearPoints
% This is a script to exercise the function:
% fcn_DataClean_findNearPoints.m
...
[nearbyIndicies, Nnearby]  = fcn_DataClean_findNearPoints(ENU_data, searchRadiusAndAngles, (fig_num));
title({sprintf('Example %.0d: showing fcn_DataClean_findNearPoints',fig_num), 'Basic test case'}, 'Interpreter','none','FontSize',12);
...
% nearbyIndicies  = fcn_DataClean_findNearPoints(tENU, searchRadiusAndAngles, ([]));
% title({sprintf('Example %.0d: showing fcn_DataClean_findNearPoints',fig_num), sprintf('File: %s',csvFile)}, 'Interpreter','none','FontSize',12);
% 
% % Was a figure created?
% assert(all(~ishandle(fig_num)));
% 
...
% nearbyIndicies  = fcn_DataClean_findNearPoints(tENU, searchRadiusAndAngles, (fig_num));
% title({sprintf('Example %.0d: showing fcn_DataClean_findNearPoints',fig_num), sprintf('File: %s',csvFile)}, 'Interpreter','none','FontSize',12);
% 
% % Was a figure created?
% assert(all(ishandle(fig_num)));
...
% if 1==1
%     fprintf(1,'\n\nComparison of fcn_DataClean_findNearPoints without speed setting (slow) and with speed setting (fast):\n');
