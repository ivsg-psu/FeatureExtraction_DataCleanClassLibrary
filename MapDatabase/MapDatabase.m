...
%                 result.laser.pose = result_pose;
            ...
            if nargin == 2 || (nargin == 3 && isfield(options.sensors,'gps_sparkfun_leftrear') && options.sensors.gps_sparkfun_leftrear == 1)
                ...
                if obj.verbose == 1
                    fprintf('Loading GPS (Rear Left SparkFun) data...\n')
                end
                ...
                fields_vtg = {'sensors_id', 'bag_files_id', 'timestamp', 'seconds', 'nanoseconds',...
                        'time', 'truetrack', 'magtrack', 'spdovergrndknots', 'spdovergrndkmph'};
                fields_gst = {'sensors_id', 'bag_files_id', 'timestamp', 'seconds', 'nanoseconds',...
                        'time', 'gpssecs', 'gpsmicrosecs', 'gpstime', 'stdmajor','stdminor',...
                        'stdori','stdlat','stdlon','stdalt'};
                result_gps_gga = fetchSensor(obj, table_gga, fields_gga, where, orderby);
                result_gps_vtg = fetchSensor(obj, table_vtg, fields_vtg, where, orderby);
                result_gps_gst = fetchSensor(obj, table_gst, fields_gst, where, orderby);
                result.GPS_SparkFun_LeftRear_GGA = result_gps_gga;
                result.GPS_SparkFun_LeftRear_VTG = result_gps_vtg;
                result.GPS_SparkFun_LeftRear_GST = result_gps_gst;
                %                 result_pose = fetchSensorPose(obj, sensor_pose_trip_id, sensor_id);
                %                 result.Garmin_GPS.pose = result_pose;
...
%                 end
            ...
            if nargin == 2 || (nargin == 3 && isfield(options.sensors,'gps_sparkfun_rightrear') && options.sensors.gps_sparkfun_leftrear == 1)
                
                if obj.verbose == 1
                    fprintf('Loading GPS (Rear Right SparkFun) data...\n')
                end
                
                %sensor_id = 8;
                table_gga = 'gps_sparkfun_rightrear_gga';
                table_vtg = 'gps_sparkfun_rightrear_vtg';
                table_gst = 'gps_sparkfun_rightrear_gst';
                % {'id','sensors_id','bag_files_id','status','service','latitude','longitude','altitude','geography',
                %  'roll','pitch','yaw','seconds','nanoseconds','position_covariance','position_covariance_type','time','timestamp','date_added'}
                fields_gga = {'sensors_id', 'bag_files_id', 'timestamp', 'seconds', 'nanoseconds',...
                        'time', 'gpssecs', 'gpsmicrosecs', 'gpstime', 'latitude', 'longitude', 'altitude',...
                ...
                result_gps_vtg = fetchSensor(obj, table_vtg, fields_vtg, where, orderby);
                result_gps_gst = fetchSensor(obj, table_gst, fields_gst, where, orderby);
                result.GPS_SparkFun_RightRear_GGA = result_gps_gga;
                result.GPS_SparkFun_RightRear_VTG = result_gps_vtg;
                result.GPS_SparkFun_RightRear_GST = result_gps_gst;
                %                 result_pose = fetchSensorPose(obj, sensor_pose_trip_id, sensor_id);
                %                 result.Garmin_GPS.pose = result_pose;
